...
def register(user_data: UserRegister, db: Session = Depends(get_db)):
    ...
    existing_user = (
        db.query(User).filter(User.staff_number == user_data.staff_number).first()
    )
    if existing_user:
        raise HTTPException(
...
def login(user_credentials: UserLogin, db: Session = Depends(get_db)):
    ...
    access_token, jti = create_access_token(
        data={"sub": user.staff_number, "role": user.role}, expires_delta=expires_delta
    )
...
def approve_user(
    user_id: int,
    update_data: Optional[UserApprovalUpdate] = None,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_admin_user),
):
    """Approves a user, enabling their account. Optionally updates department and role_designation."""
    ...
    if update_data:
        ...
        if update_data.role_designation is not None:
            user.role_designation = update_data.role_designation
